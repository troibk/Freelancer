...
public class ExcelProcessing4 extends JPanel implements ActionListener {
	/**
	 * 
	 */
			...
			if (table1Sheet == null) {
				...
				for (int i = 0; i < 3110; i++) {
					try {
						...
						Cell emailCell=table1Sheet.getRow(i).getCell(3);
						String email="";
						if(emailCell!=null) {
							email = emailCell.getStringCellValue();
							email=email.replace("[u","").replace("u'","\'").replace("\'", "").replace("]","").replace("[","");		
						}
						datalist.add(email);
						}
					} catch (Exception e) {
	...
	private void writeExcelFile() {
        ...
        try {
        	fileOut = new FileOutputStream("D:\\Freelancer\\thoan_excel\\Results\\20190601.xlsx");
	...
	public static void main(String s[]) {
		...
		ExcelProcessing4 panel = new ExcelProcessing4();
